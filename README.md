...
- ⏱ **SLA & Metrics**
  - คำนวณ `response_due_at`, `sla_due_at`, `is_sla_breached` อัตโนมัติจาก priority
  - เก็บเวลาตอบกลับครั้งแรก (`first_response_at`) และเวลาที่แก้ไขเสร็จ (`resolved_at`, `closed_at`)
  - มี Service สำหรับคำนวณ Metric เช่น MTTR, จำนวน ticket ตาม status/priority

- 🌐 **Multi-channel Ingestion API**
  - มี API `POST /api/ingest/tickets` สำหรับสร้าง ticket จากระบบภายนอก (เช่น email gateway, chat bot ฯลฯ)
  - ใช้ header `X-Integration-Token` เพื่อป้องกันการเรียก API จากภายนอกที่ไม่ได้รับอนุญาต
